...
void IMU::testImu(std::string src, std::string dist)
{
    ...
    for (int i = 1; i < imudata.size(); ++i) {
...
#else
        MotionData imupose;
        MotionData imupose_next;

        imupose = imudata[i - 1];
        imupose_next = imudata[i];


        Eigen::Vector3d w_avg = 0.5 * (imupose.imu_gyro + imupose_next.imu_gyro);
        Eigen::Quaterniond dq;
        Eigen::Vector3d dtheta_half =  w_avg * dt /2.0;
        dq.w() = 1;
        dq.x() = dtheta_half.x();
        dq.y() = dtheta_half.y();
        dq.z() = dtheta_half.z();
        ...
        Qwb = Qwb * dq;
        Eigen::Vector3d acc_avg = 0.5 * ((Qwb_pre * imupose.imu_acc + gw) + (Qwb * imupose_next.imu_acc + gw));
        Vw = Vw + acc_avg * dt;
        Pwb = Pwb + Vw * dt + 0.5 * dt * dt * acc_avg;
#endif
        /// 中值积分
